...
public class Solution {
    ...
    static int activityNotifications(int[] expenditure, int d){
        int[] spendingDays = new int[d];
        int notification = 0;

        for (int i = 0; i < expenditure.length - d - 1; i++){
            for (int j = 0; j < d; j++){
                spendingDays[j] = expenditure[i + j];
            }
            Arrays.sort(spendingDays);

            if (d % 2 == 0){
                int median = spendingDays[d / 2 + 1] + spendingDays[d / 2 + 2];

                if (median * 2 >= expenditure[d + i + 1])
                    notification++;
            }
            else{
                int median = spendingDays[(d + 1) / 2];
                if (median * 2 >= expenditure[d + i + 1])
                    notification++;
            }
        }
