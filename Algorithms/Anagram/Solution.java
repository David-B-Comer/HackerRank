...
public class Solution{
    ...
    static int anagram(String s){
        ...
        StringBuilder word1 = new StringBuilder(s.substring(0, s.length() / 2));
        StringBuilder word2 = new StringBuilder(s.substring(s.length() / 2));

        for (char c : word2.toString().toCharArray()){
            int i = word1.indexOf(String.valueOf(c));

            if (i > -1)
                word1 = word1.delete(i, i + 1);
        }
        return word1.length();
    }
